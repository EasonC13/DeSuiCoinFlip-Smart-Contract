...
module drand::coin_flip {
    use drand::drand_lib::{derive_randomness, verify_drand_signature, safe_selection};
    // Imports
    use std::vector;
    ...
    /// @param house_data: The HouseData object
    fun internal_start_game(guess: u8, user_randomness: vector<u8>, round: u64, coin: Coin<SUI>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): Game {
        // TODO: Need to assert the round haven't announced by Drand. May need the Clock Object.
    ...
    /// Stake is taken from the player's coin and added to the game's stake. The house's stake is also added to the game's stake.
    public entry fun start_game(guess: u8, user_randomness: vector<u8>, round: u64, coin: Coin<SUI>, house_data: &mut HouseData, ctx: &mut TxContext) {
        let fee_bp = base_fee_in_bp(house_data);
        let new_game = internal_start_game(guess, user_randomness, round, coin, house_data, fee_bp, ctx);
    ...
    public entry fun start_game_with_capy(_: &SuiFren<Capy>, guess: u8, user_randomness: vector<u8>, round: u64, coin: Coin<SUI>, house_data: &mut HouseData, ctx: &mut TxContext) {
    ...
    public entry fun start_game_with_bullshark(kiosk: &Kiosk, item: ID, guess: u8, user_randomness: vector<u8>, round: u64, coin: Coin<SUI>, house_data: &mut HouseData, ctx: &mut TxContext) {
    ...
    public entry fun start_game_with_dlab(kiosk: &Kiosk, item: ID, guess: u8, user_randomness: vector<u8>, round: u64, coin: Coin<SUI>, house_data: &mut HouseData, ctx: &mut TxContext) {
        // Ensure user has bullshark
    ...
    public entry fun play(
        ...
        // TODO: change m to user's randomness
        let random: u128 = 0
        let random_number = safe_selection(length, &digest, random);
        
        // TODO: convert the random number to a byte and decide if users win
